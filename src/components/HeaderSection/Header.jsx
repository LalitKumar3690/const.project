  ...
  return (
    ...
    >
      <nav className={`w-full top-0 left-0 z-50 fixed  flex items-center justify-center ${scrolling? 'bg-gradient-to-l to-white via-white from-sky-900 2xl:h-[6rem] xl:h-[5rem] lg:h-[4rem] transition-colors duration-300 ease-in-out' : 'bg-gradient-to-l to-white via-white/ from-[#3472b03b] 2xl:h-[8rem] xl:h-[7rem] lg:h-[5.5rem]'}`}>
          <div className={`w-[95%] flex justify-between rounded-md ${scrolling ? '2xl:h-[4em] xl:h-[3.5em] lg:h-[3em]' : '2xl:h-[5em] xl:h-[4.5em] lg:h-[3.5em]'}`}>
              <div className={`h-full  w-[75%] lg:w-[70%] flex justify-between rounded-md items-center`}>
                <div className={`flex items-center justify-between bg-white w-[95%] shadow-xl rounded-l-md `}>
                  <div className={`flex 2xl:w-60 xl:w-56 lg:w-48 h-full items-center  pt-1`}>
                      <img src={Logo} className={`${scrolling?'2xl:w-12 xl:w-11 lg:w-10 2xl:h-14 xl:h-11 lg:h-9':'2xl:w-20 xl:w-12 lg:w-10 2xl:h-20 xl:h-12 lg:h-10'}`}></img>
                      <img src={Logo2} className={`${scrolling?'2xl:h-9 xl:h-7 lg:h-5':'2xl:h-11 xl:h-8 lg:h-6'}`}></img>
                  </div>
                  <div className='flex 2xl:h-12 xl:h-11 lg:h-7 text-gray-600 font-semibold 2xl:gap-6 xl:gap-5 lg:gap-4'>
                      {
                        Navdata.map((value, index) => (
                          <div>
                            <div className='group h-full flex flex-col justify-center' key={value.id}>
                              <button className={`2xl:tracking-wider xl:tracking-wide 2xl:text-lg xl:text-sm lg:text-xs flex items-center font-serif 2xl:font-bold  ${value.subbuthai ? "2xl:pt-1" : "2xl:pt-0"}`} 
                              onMouseEnter={()=>setIsHovered(index)}
                              onMouseLeave={()=>setIsHovered(null)}
                              >{value.buttoname}
                                {
                                  value.subbuthai ? <span className='2xl:text-4xl xl:text-3xl lg:text-xl font-bold'>
                                    {
                              ...
                              <div className='absolute 2xl:top-16 group-hover:block hidden'
                  ...
                  <span class={`w-0 h-0 relative ${scrolling?'2xl:border-b-[4em] xl:border-b-[3em] lg:border-b-[2.8em] 2xl:border-r-[6em] xl:border-r-[5em] lg:border-r-[4em]':'2xl:border-b-[5.5em] xl:border-b-[3.3em] lg:border-b-[3em] 2xl:border-r-[7em] xl:border-r-[6em] lg:border-r-[5em]'} border-b-white border-r-transparent`}></span>
